...
{/* Left Content Column */}
  ...
  <Col xs={24} md={12}>
    ...
    | # | REQUEST | RESPONSE |
    |---|---------|----------|
    | 1 | {`https://api.gwcindia.in/v2/login?api_key=xxx`} | Redirects Goodwill Login page |
    | 2 | After Successful Goodwill Login | request_token will be passed as query string parameter to the registered redirect URL. |
    | 3 | {`https://api.gwcindia.in/v2/login-response`} with Signature | User Login Session Details |
    ...
    | URL | {`https://api.gwcindia.in/v2/login-response`} |
    | Input | YES |
