...
<Row gutter={[32, 32]} wrap>
  <Col xs={24} md={12}>
     ...
     To get characters you need to make a POST call to the following url 
      <CodeBlock language="text">{`https://api.gwcindia.in/v2/exitboorder`}</CodeBlock>
...
<CodeBlock language="bash" title="curl / cURL">
  {`curl --location --request POST 'https://api.gwcindia.in/v2/exitboorder' \
